...
public final class DictionaryMatcher implements PasswordMatcher
{
    ...
    {
        List<Character[]> uselessList = new ArrayList<>();
            ...
            for (int end = start + 1; end <= password.length(); end++)
            {
                ...
                for (Dictionary dictionary : configuration.getDictionaries())
                {
                    ...
                    String lower_part = split_password.toLowerCase();
                    {
                        ...
                        if (lower_rank != null)
                        {
                            matches.add(new DictionaryMatch(split_password, configuration, start, end - 1, lower_part, lower_rank, uselessList, dictionary.isExclusion(), false, dictionary.getDictionaryName(), 0));
                            continue;
                        }
                    }
                    ...
                    String reversed_part = new StringBuilder(lower_part).reverse().toString();
                    {
                        ...
                        {
                            matches.add(new DictionaryMatch(split_password, configuration, start, end - 1, reversed_part, reversed_rank, uselessList, dictionary.isExclusion(), true, dictionary.getDictionaryName(), 0));
                            continue;
                        }
                    }
                    ...
                    // Only if we haven't found a match yet
                    {
                        ...
                        if (dist_rank != null)
                        {
                            matches.add(new DictionaryMatch(split_password, configuration, start, end - 1, dist_val, dist_rank, uselessList, dictionary.isExclusion(), false, dictionary.getDictionaryName(), dist_min));
                            continue;
                        }
                    }
